...
const WITHDRAW_FILE = path.join(DATA_DIR, "withdraw.json");
...
export function readWithdrawLogs() {
  try {
    ensureFiles();
    const txt = fs.readFileSync(WITHDRAW_FILE, "utf8");
    return JSON.parse(txt || "[]");
  } catch (e) {
    console.error("readWithdrawLogs error", e);
    return [];
  }
}
...
export function writeWithdrawLogs(logs = []) {
  try {
    ensureFiles();
    fs.writeFileSync(WITHDRAW_FILE, JSON.stringify(logs, null, 2), "utf8");
    return true;
  } catch (e) {
    console.error("writeWithdrawLogs error", e);
    return false;
  }
}
